...
int find(S * a, int n, char * id) {
    for (int i = 0; i < n; i ++) if (! strcmp(a[i] .id, id)) return i;
    return - 1;
}
int main(void) {
    int p, m, n, k = 0, x;
    scanf("%d%d%d", & p, & m, & n);
    S * a = calloc(p + m + n, sizeof * a);
    char id[25];
    for (int i = 0; i < p; i ++) {
        scanf("%s%d", id, & x);
        strcpy(a[k] .id, id);
    ...
    for (int i = 0; i < k; i ++) if (a[i] .gp >= 200) {
        int fin = a[i] .fin < 0 ? 0 : a[i] .fin;
        if (a[i] .mid < 0 || fin >= a[i] .mid) a[i] .total = fin;
        else {
            a[i] .total = (int)(a[i] .mid * .4 + fin * .6 + 0.5);
        }
    }
    qsort(a, k, sizeof * a, cmp);
    for (int i = 0; i < k; i ++) if (a[i] .gp >= 200 && a[i] .total >= 60) printf("%s %d %d %d %d\n", a[i] .id, a[i] .gp, a[i] .mid, a[i] .fin, a[i] .total);
    free(a);
    return 0;
}
```

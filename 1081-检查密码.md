...
/*
 * 题目：1081 检查密码
 * 实现原理：扫描密码并记录长度、数字、小写、大写及特殊字符条件是否满足。
 * 复杂度：O(L)
 * 实现步骤：
 * 1. 读取 1081 检查密码 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
int main(void) {
    ...
    while (n --) {
        ...
        s[strcspn(s, "\r\n")] = 0;
        int l = strlen(s), letter = 0, digit = 0, valid = 1;
        for (int i = 0; s[i]; i ++) {
            if (isalpha((unsigned char) s[i])) letter = 1;
            else if (isdigit((unsigned char) s[i])) digit = 1;
            else if (s[i] != '.') valid = 0;
        }
        if (l < 6) puts("Your password is tai duan le.");
        else if (! valid) puts("Your password is tai luan le.");
        else if (! digit) puts("Your password needs shu zi.");
        else if (! letter) puts("Your password needs zi mu.");
